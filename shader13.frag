...
vec4 convertedData = iData*vec4(6.0, 30.0, 1.0, 1.0);
vec4 convertedData2 = convertedData+vec4(2.0, 20.0, 0.0, 0.0);
...
void main (void) {
    ...
    st = rotate2D(st,dist*convertedData2.y); // HERE y 20.
    // st = rotateTilePattern(st*2.);
    // st = rotate2D(st,PI*u_time*0.25);

    // step(st.x,st.y) just makes a b&w triangles
    // but you can use whatever design you want.
    gl_FragColor = vec4(vec3(concentricCircles(st, vec2(0.0,0.0), 5.0, 10.0)),backgroundAlpha);
}
