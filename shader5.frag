...
https://www.shadertoy.com/view/ttscWn    actual shadertoy link
*/
...
vec4 convertedData = iData*vec4(2.0, 4000.0, 1.0, 1.0);
vec4 convertedData2 = convertedData+vec4(-0.25, 320.0, 0.0, 0.0);
...
void main()
{
    ...
    for(int j=0; j<AA; j++){
        for(int i=0; i<AA; i++){
            ...
            // colors, etc.
            //float dir = mod(float(ik), 2.)<.5? -1. : 1.; // HERE!!!!!!! x
            float dir = convertedData2.x; // HERE x
            
            // Layer coloring and shading. Also made up.
            float sh = (float(iter - ik))/float(iter); // Shade.
            vec2 tuv = z/convertedData2.y; // Transformed UV coordinate. // HERE y 320.
	...
	gl_FragColor = vec4(sqrt(max(col, 0.)), backgroundAlpha );
}